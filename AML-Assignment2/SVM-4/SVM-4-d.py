...
for c_val in c_values:
    ...
    print "Number of support vectors used: %d" % (support_vecs.shape[0])
    ...
    print "Test error for C value = %.2f: %.4f" % (c_val, 1 - accuracy)
    ...
    # Finding accuracy
    accuracy = float(results.count(True))/float(len(results))
    print "Train error for C value = %.2f: %.4f" % (c_val, 1 - accuracy)